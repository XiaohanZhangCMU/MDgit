...
*** Calculates the Phonon Dispersion Curves
	...
	parameter (eps = 0.01)
C	parameter (reps = eps*alatt)
	parameter (reps = eps*4.640)
	double precision dm(3,3*nmmax)
...
C	   x(j,43) = xi(j,43) + eps
	   if(j.eq.1) then
	      x0(43)=xi(j,43)/b0+eps/b0
	   elseif(j.eq.2) then
	      y0(43)=xi(j,43)/b0+eps/b0
	   elseif(j.eq.3) then
	      z0(43)=xi(j,43)/b0+eps/b0
	   endif
