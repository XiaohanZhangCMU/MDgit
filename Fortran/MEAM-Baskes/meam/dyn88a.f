C slightly modified dyn88.f by Wei Cai
C 
C changed utils/utils.f   subroutine scale -> scale_sub
C remove contin, lastconf from dyn88.f dyn88a.f
c move common/mea/ to dyn.inc

C from original main.f

        subroutine freevel
        return
        end
        subroutine frnewton(contin)
        logical contin
        return
        end
        subroutine freepot
...
c
	block data
      ...
      include 'dyn.inc'
      include 'meam.inc'
c      logical contin,lastconf
c      character*80 printf,rstrtf,conff,meamf,forcef,meafile
c      character*24 timestr
c      common/mea/ meamf, meafile
      data eqtim/0.0/
...
c      character*80 printf,rstrtf,conff,meamf,forcef,statf,meafile
c      character*24 timestr
c	common/mea/ meamf, meafile
	common/free/ ifree
...
9100  format(' *******  dynamo version 8.7 (unix) *******'/,
     1   '$Revision: 1.30 $',/'$Date: 2004/09/29 19:26:24 $')
      ...
      if (iout.eq.1) then
         ...
         iustat=23
c         open(unit=23,file=statf,form='FORMATTED',status='UNKNOWN')
      endif
      write(6,9107)
...
c
      do 460 i=1,ntypes
         write (fid,*) 'kodes(',i,')=',kodes(i)
         if (kodes(i) .ne. 'library') go to 460
c         write(*,*) ' inter... (5) kodes=',kodes(i)
         rewind kmeamf
         read  (kmeamf,105) header
         write (fid,110) header
         read  (kmeamf,105) header
         write (fid,110) header
         read  (kmeamf,105) header
         write (fid,110) header
...
c
c         write(*,*) ' inter... (6) kodes=',kodes(i)

 410     read  (kmeamf,*) name,lattces(i),zs(i),ielement(i),amamu

c         enames(1) = 'Si'
c         write(*,*) ' inter... (7) name=',name,' enames=',enames(1)
...
c     441  format(1x,'i=',i3,' amass(i)=',e25.16)
 ...
 120  format(/,5x,'i',3x,'lattice',6x,'name',5x,'kodes')
      do 130 i=1,ntypes
      write (fid,125) i,lattces(i),enames(i),kodes(i)
...
c
 ...
 150  format(/,2x,'i',5x,'esubs(i,j)')
      do 160 i=1,ntypes
      write (fid,155) i,(esubs(i,j),j=1,ntypes)
...
c
 ...
 165  format(/,2x,'i',5x,'alphas(i,j)')
	...
	write(fid,*) 'unknown ibar', ibarr(i), ' for element ',i
	stop
	endif
      write (fid,170) i,(alphas(i,j),j=1,ntypes)
...
c
 ...
 180  format(/,2x,'i',4x,'betas(i,j)')
      do 190 i=1,4
      write (fid,185) i,(betas(i,j),j=1,ntypes)
...
c
 ...
 195  format(/,2x,'i',8x,'ts(i,j)')
      do 205 i=1,4
      write (fid,200) i,(ts(i,j),j=1,ntypes)
...
c
 ...
 210  format(/,2x,'i',4x,'deltas(i,j)')
      do 220 i=1,ntypes
      write (fid,215) i,(deltas(i,j),j=1,ntypes)
...
c
 ...
 297  format(/,2x,'i',7x,'repuls(i,j)')
      do 313 i=1,ntypes
      write (fid,300) i,(repuls(i,j),j=1,ntypes)
 313  continue
      write (fid,298) 
 298  format(/,2x,'i',7x,'attrac(i,j)')
      do 314 i=1,ntypes
      write (fid,300) i,(attrac(i,j),j=1,ntypes)
 314  continue
c
	if(nn) itable=1
c	write(*,*) '2NN: itable =',itable
c
      write (fid,135)
...
c   alats(i)  : the lattice constant of element i    
c   omegas(i) : the atomic volume of element i
c   res(i,i)  : the nearest element distance 
c   a2nn : the ratio between the 2nd and 1st 
c          nearest neighbor distances
c   b2nn : the screening const. on the 2nd nearest neighbors
...
c     dia
	...
	write(fid,*) 'can not do 2NN MEAM for dia'
	stop
	endif
c
c     hcp (ideal c/a)
 ...
 240  format(1x,'Error in subroutine meamdat, lattce=',a3)
	...
	if(.not.nn .or. ibarr(i).ge.0) then
     ...
     1    nn,1.)
        ...
        write (fid,140) i,alats(i),asubs(i),zs(i),z0s(i),rozros(i)
...
c
 ...
 265  format(/,2x,'i',5x,'esubs(i,j)')
      do 275 i=1,ntypes
      write (fid,270) i,(esubs(i,j),j=1,ntypes)
...
c
 ...
 280  format(/,2x,'i',5x,'alphas(i,j)')
      do 290 i=1,ntypes
      write (fid,285) i,(alphas(i,j),j=1,ntypes)
...
c
 ...
 295  format(/,2x,'i',7x,'res(i,j)')
      do 305 i=1,ntypes
      write (fid,300) i,(res(i,j),j=1,ntypes)
...
c
 ...
 296  format(/,2x,'i',7x,'all(i,j)')
      do 306 i=1,ntypes
      write (fid,301) i,(all(i,j),j=1,ntypes)
 301  format(1x,i2,5x,4a10)
 306  continue 
c
      alat=alats(1)
c
	write(fid,*) ' For screening:'
	do 307 i=1,ntypes
	write(fid,*) ' type ',i
	write(fid,*) ' cmax'
	do 308 j=1,ntypes
	write(fid,9331) (cmax(j,i,k),k=1,ntypes)
  308	continue
	write(fid,*) ' cmin'
	do 309 j=1,ntypes
	write(fid,9331) (cmin(j,i,k),k=1,ntypes)
  309	continue
9331	format(10f8.2)
  307	continue
400	continue
...
c
	if(itable.gt.0) then
	write(fid,*) 'setting up table for phi'
	do 407 i=1,ntypes
	if(nn) then
	nmax=10
        write(fid,*) 'calc. 2NN MEAM with a2nn,b2nn=',a2nn(i),b2nn(i)
	else
	nmax=0
	endif
	do 407 k=1,ntable
