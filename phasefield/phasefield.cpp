...
void PhaseFieldFrame::initparser()
{
    ...
    bindvar("timestep_ela",&timestep_ela,DOUBLE);
    ...
    bindvar("L_vol_incr0",&L_vol_incr0,DOUBLE);
    ...
    bindvar("epseigfile",epseigfile,STRING);
    bindvar("epsfile",epsfile,STRING);
    bindvar("sigfile",sigfile,STRING);
    bindvar("dispfile",dispfile,STRING);
    bindvar("Kfile",Kfile,STRING);
...
#ifdef _ELASTIC
    // A general way to write stiffness matrix
    for (i=0;i<3;i++)
        for (j=0;j<3;j++)
          for (k=0;k<3;k++)
              for (l=0;l<3;l++)
             {
                 sprintf(s,"Cstiff_%d%d%d%d",i,j,k,l);
                 bindvar(s,&(_Cstiff[i][j][k][l]),DOUBLE);
             }
#endif
...
#ifdef _ELASTIC
    /* Set initial fields */
    bindcommand(name,"setdisp",setdisp());
    bindcommand(name,"setinitialphi",setinitialphi());
    bindcommand(name,"setK",setK());
    bindcommand(name,"setepseig",setepseig());
#endif
...
#ifdef _ELASTIC
    bindcommand(name,"readepseig",readepseig(epseigfile));
    bindcommand(name,"readdisp",readdisp(dispfile));
    bindcommand(name,"readK",readK(Kfile));
    bindcommand(name,"writeeps",writeeps(epsfile));
    bindcommand(name,"writesig",writesig(sigfile));
    bindcommand(name,"writedisp",writedisp(dispfile));
#endif
...
/* For MPI implementation based on the StencilToolkit */
    ...
    for (i=0; i<3; i++)
    {
        Realloc(_udisp[i], REAL_T, Ntot);
        memset(_udisp[i], 0, sizeof(REAL_T) * Ntot);
        
        Realloc(_dEdu[i], REAL_T, Ntot);
        memset(_dEdu[i], 0, sizeof(REAL_T) * Ntot);
    }
    
    for (i=0; i<3; i++)
        for (j=0; j<3; j++)
        {
            Realloc(_eps_eigen[i][j], REAL_T, Ntot);
            memset(_eps_eigen[i][j], 0, sizeof(REAL_T) * Ntot);
            
            Realloc(_eps[i][j], REAL_T, Ntot);
            memset(_eps[i][j], 0, sizeof(REAL_T) * Ntot);
            
            Realloc(_sig[i][j], REAL_T, Ntot);
            memset(_sig[i][j], 0, sizeof(REAL_T) * Ntot);
            
            Realloc(_sigt[i][j], REAL_T, Ntot);
            memset(_sigt[i][j], 0, sizeof(REAL_T) * Ntot);
            
        }
#endif

    // for single phase field
#ifdef _ELASTIC
    Realloc(_dEdphi, REAL_T, Ntot);
    memset(_dEdphi, 0, sizeof(REAL_T) * Ntot);
#endif
    ...
    Realloc(_K,REAL_T,Ntot);
    memset(_K,0,sizeof(REAL_T)*Ntot);
          ...
          for(i=0;i<_NX;i++)
            for(j=0;j<_NY;j++)
                for(k=0;k<_NZ;k++)
                {
                    ...
                    {        
                        _PHI[1][i*_NY*_NZ+j*_NZ+k]=1.0-1e-6;
                    }     
                    else
                    {   
                        _PHI[1][i*_NY*_NZ+j*_NZ+k]=1e-6;
                    }
                    _PHI[2][i*_NY*_NZ+j*_NZ+k] = 5e-7;
    ...
    {   
        ...
        for(i=0;i<_NX;i++)
          for(j=0;j<_NY;j++)
              for(k=0;k<_NZ;k++)
              {  
                 if (SQR(i+1-_NX*input[4])+SQR(j+1-_NY*input[5])+SQR(k+1-_NZ*input[2])<SQR(max(_NX,_NY)*input[1]))
                    _PHI[0][i*_NY*_NZ+j*_NZ+k] = 1.0-1e-6;
                 else
                    _PHI[0][i*_NY*_NZ+j*_NZ+k] = 0.0+1e-6;
...
void PhaseFieldFrame::run()
{
    ...
    {   
        ...
        liquid_volume = 0; com_x = 0; com_y = 0; com_x0 = 0; com_y0 = 0;
        // trying to set the external force to be 0 (testing!!)
if ( model_type != 20 )
{
        for (i = x_begin; i < x_end; i++)
            for (j = y_begin; j < y_end; j++)
...
#ifdef _STK_MPI
                 double phi_0 = (*s_PHI[0])(k, j, i);
                 com_x0 += (tanh((phi_0-0.5)/0.10)/2 + 0.5)*(*s_matrix_x)(k,j,i)/liquid_volume;
                 com_y0 += (tanh((phi_0-0.5)/0.10)/2 + 0.5)*(*s_matrix_y)(k,j,i)/liquid_volume;
#else
                int idx = i*_NY*_NZ + j*_NZ + k;
                 com_x0 += (tanh((_PHI[0][idx]-0.5)/0.10)/2 + 0.5)*matrix_x[idx]/liquid_volume;
                 com_y0 += (tanh((_PHI[0][idx]-0.5)/0.10)/2 + 0.5)*matrix_y[idx]/liquid_volume;
#endif
            }

#ifdef _STK_MPI
    _node->reduce(STK_SUM, STK_DOUBLE, &com_x0);
    _node->reduce(STK_SUM, STK_DOUBLE, &com_y0);
#endif        
}
        ...
        {
#ifdef _ELASTIC
            for (int substep = 0; substep < elasteps; substep++)
            {
               FreeEnergy_elastic();
                ...
                for (i = x_begin; i < x_end; i++)
                    for (j = y_begin; j < y_end; j++)
                        ...
                        {
                            int idx = i*_NY*_NZ + j*_NZ + k;
                            _udisp[0][idx] -= _dEdu[0][idx]*timestep_ela;
                            _udisp[1][idx] -= _dEdu[1][idx]*timestep_ela;
                            _udisp[2][idx] -= _dEdu[2][idx]*timestep_ela;
                        }
            }
...
if ( model_type != 20 )
{
...
else if (model_type == 20)
    {
        ...
        if (model_type != 100)
        {
            // calculate the increment in the center of mass position per step
            com_y_incr = com_y - com_y_old;
            // record the old center of mass position of y
            com_y_old = com_y;
            ...
            if (create_vapor == 1)
            {
               ...
               if (_NZ - droplet_height <= 30)
               {               
                   /* remove the possbile solid  phase */
                    for(i=0;i<_NX;i++)
                       for(j=0;j<_NY;j++)
                           for(k= droplet_height+10;k<_NZ;k++)
                           {
...
#define _R _ROT_MATRIX 
     ...
     /* coupling with elasticity, only working for n = 1 */
#ifdef _ELASTIC
    if(model_type==100)
    {
        for(i=0;i<_NX;i++)
    ...
    {
        if (dynamics_type == 0) /* growth without constraint */
        {
            for(ind=0;ind<_NX*_NY*_NZ;ind++)
            {
                _dPHIdt[n][ind] = -1.0 * _K[ind] * _dFdPHI[n][ind];
            }
        }
        else if (dynamics_type == 1) /* CH equation of motion */
        {
            for(i=0;i<_NX;i++)
            {
                ip = (i+1)%_NX;
                ...
                for (j=0;j<_NY;j++)
                {
                    ...
                    for (k=0;k<_NZ;k++)
                    {
                        ...
                        _d2dFdPHI[n][ind] = (_K[ip*_NY*_NZ+j*_NZ+k]-_K[im*_NY*_NZ+j*_NZ+k])
                        * (_dFdPHI[n][ip*_NY*_NZ+j*_NZ+k]-_dFdPHI[n][im*_NY*_NZ+j*_NZ+k])/h2/4.0
                        + (_K[i*_NY*_NZ+jp*_NZ+k]-_K[i*_NY*_NZ+jm*_NZ+k])
                        * (_dFdPHI[n][i*_NY*_NZ+jp*_NZ+k]-_dFdPHI[n][i*_NY*_NZ+jm*_NZ+k])/h2/4.0
                        + (_K[i*_NY*_NZ+j*_NZ+kp]-_K[i*_NY*_NZ+j*_NZ+km])
                        * (_dFdPHI[n][i*_NY*_NZ+j*_NZ+kp]-_dFdPHI[n][i*_NY*_NZ+j*_NZ+km])/h2/4.0
                        + _K[ind]*(_dFdPHI[n][ip*_NY*_NZ+j*_NZ+k] + _dFdPHI[n][im*_NY*_NZ+j*_NZ+k]
                         +_dFdPHI[n][i*_NY*_NZ+jp*_NZ+k] + _dFdPHI[n][i*_NY*_NZ+jm*_NZ+k]
                         +_dFdPHI[n][i*_NY*_NZ+j*_NZ+kp] + _dFdPHI[n][i*_NY*_NZ+j*_NZ+km]
                         -_dFdPHI[n][ind] * 6.0 ) / h2;
                    }
            ...
            for(ind=0;ind<_NX*_NY*_NZ;ind++)
            _dPHIdt[n][ind] = 1.0 * _d2dFdPHI[n][ind];

        }
        else if (dynamics_type == 2) /* GL with volume constraint */
        {
            sum_K = 0; sum_K_dFdphi = 0;
            
            for(ind=0;ind<_NX*_NY*_NZ;ind++)
            {
                sum_K += _K[ind];
                sum_K_dFdphi += _K[ind] * _dFdPHI[n][ind];
            }
            
            for(ind=0;ind<_NX*_NY*_NZ;ind++)
            {
                _dPHIdt[n][ind] = -1.0*_K[ind]*(_dFdPHI[n][ind]-sum_K_dFdphi)/sum_K;
            }
        }
        else
        {
...
double PhaseFieldFrame::FreeEnergy_multi() /* multi-phase field model (MPF) */
{
    ...
    double L_vol_incr, vol_incr, x_incr, y_incr;
    ...
    if (dynamics_type == 4 || dynamics_type == 5 || dynamics_type == 6 || dynamics_type == 7 || dynamics_type == 10)
    {
        // _MU[1] = 0;
        _MU[0] = _MU[1];
        _MU[2] = _MU[1];
    ...
    /* adjust time step at every step */
    timestep = dtmax/fmax(_G*dtmax/dphimax,1);
    L_vol_incr = L_vol_incr0/timestep; 
    vol_incr = vol_incr0/timestep;
    x_incr = x_incr0*liquid_volume/timestep;
    y_incr = y_incr0*liquid_volume/timestep;
    
    
    if (dynamics_type != 2 && dynamics_type != 3 && dynamics_type != 4 && dynamics_type != 5 && dynamics_type != 6 && dynamics_type != 7 && dynamics_type != 8 && dynamics_type != 10)
    {
        for(n=0;n<num_fields;n++)
        {
         if (dynamics_type == 1) /* Cahn-Hillard equation */
    ...
    {   
        ...
        {   
            ...
            A_31 += (matrix_x[ind]-com_x0)*_dGLdmuL[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_32 += (matrix_x[ind]-com_x0)*_dGLdmuV[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_33 += matrix_x[ind]*(matrix_x[ind]-com_x0)*_dGLdmuL[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_34 += (matrix_x[ind]-com_x0)*matrix_y[ind]*_dGLdmuL[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_41 += (matrix_y[ind]-com_y0)*_dGLdmuL[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_42 += (matrix_y[ind]-com_y0)*_dGLdmuV[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_43 += matrix_x[ind]*(matrix_y[ind]-com_y0)*_dGLdmuL[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            A_44 += matrix_y[ind]*(matrix_y[ind]-com_y0)*_dGLdmuL[ind]*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
            B_1 += -(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)))*_dPHIdt0[0][ind];
            B_2 += -(5.0-5.0*SQR(tanh(10*_PHI[2][ind]-5.0)))*_dPHIdt0[2][ind];
            B_3 += -(matrix_x[ind]-com_x0)*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)))*_dPHIdt0[0][ind];
            B_4 += -(matrix_y[ind]-com_y0)*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)))*_dPHIdt0[0][ind];
        }
    ...
    // not finished yet
    else if (dynamics_type == 10) // force the NW to grow at a minimum droplet velocity
    {
        if ( fabs(com_y_incr) < fabs(y_incr0) ) // force the NW to grow along certain path
        {
            /* Update _mprime */
            for (ind=0; ind<_NX*_NY*_NZ; ind++)
            {
        ...
        else // loose the contraint on center of mass position
        {
            _MU[1] = 0; Fext_x = 0; Fext_y = 0;
            /* Update _mprime */
            for (ind=0; ind<_NX*_NY*_NZ; ind++)
            {
                _dGLdmuL[ind] = - _K_LS[ind]*(Mob_GL/2+Mob_LS)*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)))
...
#ifdef _ELASTIC
double PhaseFieldFrame::FreeEnergy_elastic()
{
    int i, j, k, m, n, p, q, ip, im, jp, jm, kp, km, ind;
    double ug_xx, ug_xy, ug_xz, ug_yx, ug_yy, ug_yz, ug_zx, ug_zy, ug_zz;
    double tmp;
    ...
    for(i=0;i<_NX;i++)
    {
        ...
        for (j=0;j<_NY;j++)
        {
            ...
            for (k=0;k<_NZ;k++)
            {
                ...
                ind = i*_NY*_NZ+j*_NZ+k;
                ug_xx = (_udisp[0][ip*_NY*_NZ+j*_NZ+k] - _udisp[0][im*_NY*_NZ+j*_NZ+k]) / (2.0*gridsize);
                ug_yx = (_udisp[1][ip*_NY*_NZ+j*_NZ+k] - _udisp[1][im*_NY*_NZ+j*_NZ+k]) / (2.0*gridsize);
                ug_zx = (_udisp[2][ip*_NY*_NZ+j*_NZ+k] - _udisp[2][im*_NY*_NZ+j*_NZ+k]) / (2.0*gridsize);

                ug_xy = (_udisp[0][i*_NY*_NZ+jp*_NZ+k] - _udisp[0][i*_NY*_NZ+jm*_NZ+k]) / (2.0*gridsize);
                ug_yy = (_udisp[1][i*_NY*_NZ+jp*_NZ+k] - _udisp[1][i*_NY*_NZ+jm*_NZ+k]) / (2.0*gridsize);
                ug_zy = (_udisp[2][i*_NY*_NZ+jp*_NZ+k] - _udisp[2][i*_NY*_NZ+jm*_NZ+k]) / (2.0*gridsize);

                ug_xz = (_udisp[0][i*_NY*_NZ+j*_NZ+kp] - _udisp[0][i*_NY*_NZ+j*_NZ+km]) / (2.0*gridsize);
                ug_yz = (_udisp[1][i*_NY*_NZ+j*_NZ+kp] - _udisp[1][i*_NY*_NZ+j*_NZ+km]) / (2.0*gridsize);
                ug_zz = (_udisp[2][i*_NY*_NZ+j*_NZ+kp] - _udisp[2][i*_NY*_NZ+j*_NZ+km]) / (2.0*gridsize);

                /* calculate strain field */
                _eps[0][0][ind] = ug_xx;
                _eps[1][1][ind] = ug_yy;
                _eps[2][2][ind] = ug_zz;
                _eps[0][1][ind] = (ug_xy + ug_yx)/2;
                _eps[1][0][ind] = _eps[0][1][ind];
                _eps[0][2][ind] = (ug_xz + ug_zx)/2;
                _eps[2][0][ind] = _eps[0][2][ind];
                _eps[1][2][ind] = (ug_yz + ug_zy)/2;
                _eps[2][1][ind] = _eps[1][2][ind];
                
                /* reset stress value at the beginning of each step */
                for (m=0;m<3;m++)
                    for (n=0;n<3;n++)
                    {
                        _sigt[m][n][ind] = 0;
                ...
                for (m=0;m<3;m++)
                    for (n=0;n<3;n++)
                    {
                        for (p=0;p<3;p++)
                            for (q=0;q<3;q++)
                            {
                                _sigt[m][n][ind] = _sigt[m][n][ind] + _Cstiff[m][n][p][q]*(_eps[p][q][ind]-_eps_eigen[p][q][ind]);
                            }
                        _sig[m][n][ind] = _sigt[m][n][ind]*(tanh((_PHI[0][ind]-0.5)/0.10)/2+0.5);
                        _E += _sig[m][n][ind]*(_eps[m][n][ind]-_eps_eigen[m][n][ind])/2;
                        tmp += _sigt[m][n][ind]*(_eps[m][n][ind]-_eps_eigen[m][n][ind])/2;
                    }
                
                _dEdphi[ind] = tmp*(5.0-5.0*SQR(tanh(10*_PHI[0][ind]-5.0)));
                
                _dEdu[0][ind] = -(_sig[0][0][ip*_NY*_NZ+j*_NZ+k] - _sig[0][0][im*_NY*_NZ+j*_NZ+k] + _sig[0][1][i*_NY*_NZ+jp*_NZ+k]
                              - _sig[0][1][i*_NY*_NZ+jm*_NZ+k] + _sig[0][2][i*_NY*_NZ+j*_NZ+kp] - _sig[0][2][i*_NY*_NZ+j*_NZ+km] )/(2.0*gridsize);
                _dEdu[1][ind] = -(_sig[1][0][ip*_NY*_NZ+j*_NZ+k] - _sig[1][0][im*_NY*_NZ+j*_NZ+k] + _sig[1][1][i*_NY*_NZ+jp*_NZ+k]
                              - _sig[1][1][i*_NY*_NZ+jm*_NZ+k] + _sig[2][2][i*_NY*_NZ+j*_NZ+kp] - _sig[2][2][i*_NY*_NZ+j*_NZ+km] )/(2.0*gridsize);
                _dEdu[2][ind] = -(_sig[2][0][ip*_NY*_NZ+j*_NZ+k] - _sig[2][0][im*_NY*_NZ+j*_NZ+k] + _sig[2][1][i*_NY*_NZ+jp*_NZ+k]
                              - _sig[2][1][i*_NY*_NZ+jm*_NZ+k] + _sig[2][2][i*_NY*_NZ+j*_NZ+kp] - _sig[2][2][i*_NY*_NZ+j*_NZ+km] )/(2.0*gridsize);
            }
        }
    }
...
{ /* Displacement */
    ...
    DUMP("setdisp finished");
}

void PhaseFieldFrame::setinitialphi()
{ /* phase fields */
  int n, i, j, k, ip, im, jp, jm, kp, km;
  INFO_Printf("num_fields = %d, NX = %d, NY = %d, NZ = %d\n", num_fields, _NX, _NY, _NZ);
  for(n=0;n<num_fields;n++) {
    for(i=0;i<_NX;i++) {
      ip = (i+1)%_NX;
      im = (i-1+_NX)%_NX;
      for (j=0;j<_NY;j++) {
        ...
        for (k=0;k<_NZ;k++) {
          kp = (k+1)%_NZ;
          km = (k-1+_NZ)%_NZ;
          if ((i-_NX/2)* (i-_NX/2) + (j-_NY/2) * (j-_NY/2) + (k-_NZ/2) * (k-_NZ/2)< 10 )
            _PHI[n][i*_NY*_NZ+j*_NZ+k] = 1.0;
       } } } }

  DUMP("setPHI finished");
}

void PhaseFieldFrame::setepseig()
{ /* eigen strain field */
    ...
    DUMP("setepseig finished");
}
#endif

void PhaseFieldFrame::setK()
...
/********************************************************************/
#ifdef _ELASTIC
int PhaseFieldFrame::readdisp(const char* fname)
{
    /* the format of the disp data file should be a 1d array. */
    int n, i;
    double tmp;
    ...
    Free(buffer);
    DUMP("readdisp finished");
    return 0;
}



int PhaseFieldFrame::readepseig(const char* fname)
{
    /* the format of the disp data file should be a 1d array. */
    int m, n, i;
    double tmp;
    ...
    for(m=0;m<3;m++)
        for(n=0;n<3;n++)
            for(i=0;i<Ntot;i++)
            {
                ...
                sscanf(q, "%le",&tmp);
                _eps_eigen[m][n][i] = tmp;
            }
    
    Free(buffer);
    DUMP("readepseig finished");
    return 0;
}

int PhaseFieldFrame::readK(const char* fname)
{
    /* the format of the disp data file should be a 1d array. */
    int i;
    double tmp;
...
void PhaseFieldFrame::writeeps(char *fname)
{
    int iter;
    FILE *fp;
    fp = fopen(fname,"w");
    for(iter=0;iter<Ntot;iter++)
    {
        fprintf(fp,"%le %le %le %le %le %le %le %le %le\n",
                _eps[0][0][iter],_eps[0][1][iter],_eps[0][2][iter],
                _eps[1][0][iter],_eps[1][1][iter],_eps[2][1][iter],
                _eps[2][0][iter],_eps[2][1][iter],_eps[2][2][iter]);
    }
    fclose(fp);
}

void PhaseFieldFrame::writesig(char *fname)
{
    int iter;
    FILE *fp;
    fp = fopen(fname,"w");
    ...
    {
        fprintf(fp,"%le %le %le %le %le %le %le %le %le\n",
                _sig[0][0][iter],_sig[0][1][iter],_sig[0][2][iter],
                _sig[1][0][iter],_sig[1][1][iter],_sig[2][1][iter],
                _sig[2][0][iter],_sig[2][1][iter],_sig[2][2][iter]);
    }
    fclose(fp);
}

void PhaseFieldFrame::writedisp(char *fname)
{
    int iter;
    FILE *fp;
    fp = fopen(fname,"w");
    ...
    {
        fprintf(fp,"%le %le %le\n",
                _udisp[0][iter],_udisp[1][iter],_udisp[2][iter]);
    }
    fclose(fp);
}
#endif
...
int PropFile::writeentry(void *p)
{
    char tmp[500];
    PhaseFieldFrame &d=*((PhaseFieldFrame *)p);
    sprintf(tmp,"%10d %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e\n",
            d.curstep,d._F,d._Fraw,d._G, d.Fext_x, d.Fext_y, d.com_y, d._MU[2]);
    *f<<tmp;
    return 0;
}
