...
#
# run as (in md++.pbs):
#  bin/fem_intel scripts/work/fe-buckle/fe-buckle.tcl 0
...
MD++ setnolog
MD++ setoverwrite
MD++ dirname = runs/fe-buckle-$status
MD++ NNM = 200
}
...
if { $status == 0 } {
  ...
  if { $pbid == 1 } { # 5element
    MD++ fem_coeff_file = "../../scripts/work/fe-buckle/membrane2d-5element-CPE4-Q4.dat"  read_fem_coeff
    MD++ elements_file = "../../scripts/work/fe-buckle/membrane2d-5element.ele"  read_elements
    MD++ incnfile = "../../scripts/work/fe-buckle/membrane2d-5element.cn" readcn
    MD++ map23 = \[ 0 1 \] 
    MD++ RtoRref
    #setup_window
    openwindow
...
#     MD++ R(20) = 1   #z(6)
  ...
  if { $pbid == 2 } { # 8x5 elements
    ...
    MD++ fem_coeff_file = "../../scripts/work/fe-buckle/membrane2d-8x5element-CPE4-Q4.dat"  read_fem_coeff
    MD++ elements_file = "../../scripts/work/fe-buckle/membrane2d-8x5element.ele"  read_elements
    MD++ incnfile = "../../scripts/work/fe-buckle/membrane2d-8x5element.cn" readcn
    MD++ fem_bdy_nodes_file = "../../scripts/work/fe-buckle/membrane2d-8x5element.bdy" read_bdy_nodes
    MD++ map23 = \[ 0 1 \] 
...
#    puts bdy_group
  ...
  if { $pbid == 4 } {
    ...
    # 0:left, 2:right, 1:low, 3:top	
    MD++ fem_coeff_file = "../../scripts/work/fe-buckle/membrane2d-5element-CPE4-Q4.dat"  read_fem_coeff
    MD++ elements_file = "../../scripts/work/fe-buckle/membrane2d-5element.ele"  read_elements
    MD++ incnfile = "../../scripts/work/fe-buckle/membrane2d-5element.cn" readcn
    MD++ fem_bdy_nodes_file = "../../scripts/work/fe-buckle/membrane2d-5element.bdy" read_bdy_nodes
    MD++ map23 = \[ 0 1 \] 
...
#   setup_window
	...
	if { $bid == 2 } {
		MD++ group($bnd) = 2
		puts "bnd $bnd is set to force group 2"
	    }
	if { $bid == 3  } {
#	    if { $xb >= -0.3 && $xb <= 0.3 } {
#		    MD++ group($bnd) = 1
#		    puts "bnd $bnd is set to force group 1"
#		}

		for { set subiter 0 } { $subiter < $n_bdy_nodes } { incr subiter 1 } {
		    set subbid [MD++_Get bTags($subiter)]
		    if { $subbid == 2 } {
			    MD++ group($bnd) = 1
			}
		}
...
#add a perturbation -- seem not work
        if { 0 } {
	    if { $bid == 1 } {
	    if { $xb <= 0.1 && $xb >=-0.1 } {		    
		    set y_curr [ MD++_Get R([expr $bnd *3+1]) ]
		    set x_curr [ MD++_Get R([expr $bnd *3 ]) ]
		    set y_new [ expr $y_curr + 1 ]
		    MD++ R([expr $bnd * 3 + 1 ]) = $y_new
		}
	    }
