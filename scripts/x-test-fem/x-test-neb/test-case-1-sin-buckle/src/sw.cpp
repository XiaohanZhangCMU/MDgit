...
void SWFrame::initparser()
{
    ...
    /* input */
    bindvar("sw3b_multiply",&_SW3B_MUL,DOUBLE);
    bindvar("sw2b_multiply",&_SW2B_MUL,DOUBLE);
    bindvar("tote2",&tote2,DOUBLE);
    bindvar("tote3",&tote3,DOUBLE);
}
...
void SWFrame::SWITCHpotential_user(double lambda)
{
    stillinger_weber_switch_3body(lambda);
}
...
{             
    ...
    if(iatom>=_NP) n1=_NP+1;     /* test particle */
    
    for(ipt=n0;ipt<n1;ipt++)
    {
        if(ipt>=_NP) ipt=iatom; /* test particle */
        
        /* if fixed == -1, simply ignore this atom */
        if(fixed[ipt]==-1) continue;
        
        relevant=0;
        if(ipt==iatom) relevant=1;
#if 1 /* also need to consider 3-body terms when iatom is not at the center */
        if(!relevant)
        {
            for(j=0;j<nn[ipt];j++)
            {
                jpt=nindex[ipt][j];
                if(jpt==iatom) relevant=1;
            }
        }
#endif
