...
class TestOrg : public Organizer
{
public:
    ...
    TestOrg():intval(0),x(0),y(0),z(0){initvars();}
    virtual ~TestOrg(){};
    virtual void initparser()
    {
        bindvar("intval",&intval,INT);
        bindvar("x",&x,DOUBLE);
        bindvar("y",&y,DOUBLE);
        bindvar("z",&z,DOUBLE);
        bindvar("str",str,STRING);
    }

    virtual int exec(char *name)
    {
        if(!diropened)
            if(strcmp(name,"setoverwrite")!=0)
                if(strcmp(name,"setnolog")!=0)
                    ERROR("cannot run this command before dirname is set");
        
        if(Organizer::exec(name)==0) return 0;
        bindcommand(name,"printall",printall());
        bindcommand(name,"openfile",openfile());
        bindcommand(name,"closefile",closefile());
    ...
    void openfile()
    {
        lfile=LFile::Open(str,LFile::O_Read);
    }
...
{
    test.initvars();
    test.parse(SCParser::getfilehandler(argc,argv));
    return 0;
}
