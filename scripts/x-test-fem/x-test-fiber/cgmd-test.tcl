...
MD++ setnolog
MD++ setoverwrite
exec mkdir -p runs/fiber/Samp800_Len400_Node80_Ini_Cycle_${nfiber}_${ncase}
...
# in GPa (100-5000), a smaller Y modulus makes it softer. 5000 originally

set Youngs_Modulus 500
...
#
MD++ input = \[ $ntype $nfiber $chainlen $bondlen $Lx $Ly $Lz $orient_pref \]
MD++ makefibers 
...
#Plot Settings
#
atomradius = 10 bondradius = 5 bondlength = 0
...
conj_ftol = 1e-4 conj_itmax = 100000 conj_fevalmax = 5000000
# originally 1000000
conj_fixbox = 0  conj_fixboxvec = [ 0 1 1
                                    1 0 1
                                    1 1 0 ]
#conj_fixbox = 0  conj_fixboxvec = [ 1 0 1
#                                    0 0 1
#                                    1 1 1 ]
...
MD++ incnfile = "../../../scripts/work/cgmd2/membrane2d-8x5element.cn" read2cn
MD++ map23 = \[ 0 2 \]
MD++ fem_coeff_file = "../../../scripts/work/cgmd2/membrane2d-8x5element-CPE4-Q4.dat"  read_fem_coeff
MD++ elements_file = "../../../scripts/work/cgmd2/membrane2d-8x5element.ele"  read_elements
MD++ fem_bdy_nodes_file = "../../../scripts/work/cgmd2/membrane2d-8x5element.bdy" read_bdy_nodes
MD++ input = $NP0 shift_fem_node_id 
} else {
MD++ incnfile = "../../../scripts/work/cgmd2/membrane2d-5element.cn" read2cn
MD++ map23 = \[ 0 2 \]
MD++ fem_coeff_file = "../../../scripts/work/cgmd2/membrane2d-5element-CPE4-Q4.dat"  read_fem_coeff
MD++ elements_file = "../../../scripts/work/cgmd2/membrane2d-5element.ele"  read_elements
MD++ fem_bdy_nodes_file = "../../../scripts/work/cgmd2/membrane2d-5element.bdy" read_bdy_nodes
MD++ input = $NP0 shift_fem_node_id 
}
...
#	  MD++ testpotential
	  ...
	  MD++ finalcnfile = "fibers-relaxed-fixbox.cn"  writeall=1 writecn
...
#	  MD++ dockbeads
  ...
  MD++ incnfile = "fibers-relaxed-fixbox.cn" readcn
  MD++ input = \[ $ntype $nfiber $chainlen $bondlen $Lx $Ly $Lz \] linkfibers writeall = 100
}
...
#  openwindow
  ...
  set dirname "../Samp800_Len400_Node80_Ini_Cycle_${nfiber}_${ncase}/"
  set fibname "fibers-relaxed-fixbox.cn"
  set fname $dirname$fibname
  MD++ incnfile =  $fname readcn
  MD++ elements_file = "../../../scripts/work/cgmd2/membrane2d-8x5element.ele"  read_elements
  ...
  # unloading1
  for {set y 1} {$y<=$nMax1} {incr y} {
    set fibname fibers-unload1-$y.cn
    set fname $dirname$fibname
    puts "fname = $fname"
    MD++ incnfile =  $fname readcn
    MD++ input = \[ $ntype $nfiber $chainlen $bondlen $Lx $Ly $Lz \] linkfibers writeall=100
  ...
  # unloading2
  for {set y 1} {$y<=$nMax2} {incr y} {
    set fibname fibers-unload2-$y.cn
    set fname $dirname$fibname
    puts "fname = $fname"
    MD++ incnfile =  $fname readcn
    MD++ input = \[ $ntype $nfiber $chainlen $bondlen $Lx $Ly $Lz \] linkfibers writeall=100
  ...
  # unloading3
  for {set y 1} {$y<=$nMax3} {incr y} {
    set fibname fibers-unload3-$y.cn
    set fname $dirname$fibname
    puts "fname = $fname"
    MD++ incnfile =  $fname readcn
    MD++ input = \[ $ntype $nfiber $chainlen $bondlen $Lx $Ly $Lz \] linkfibers writeall=100
...
#  MD++ incnfile = "../Samp400_Node20_Cyclep2p4p6_${nfiber}_${ncase}/fibers-adh-20.cn" readcn 
  MD++ incnfile = "../Samp800_Len400_Node80_Ini_Cycle_${nfiber}_${ncase}/fibers-relaxed-fixbox.cn" readcn 
