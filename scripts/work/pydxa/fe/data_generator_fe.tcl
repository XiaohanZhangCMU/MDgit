...
potfile = "~/Planet/Libs/MD++UMB.svn3/potentials/fe_pot"
eamgrid = 5000 readeam NNM = 200
} }

# make sure the coordinate is right hand sided.
proc make_perfect_crystal { status nx ny nz } {
    MD++ crystalstructure = body-centered-cubic latticeconst = 2.8665 #(A) for Cu
    if { $status == 0 } { 
      MD++ latticesize = \[  1 1 -2  $nx  1 -1 0  $ny  0.5 0.5 0.5  $nz \]
    } else { 
