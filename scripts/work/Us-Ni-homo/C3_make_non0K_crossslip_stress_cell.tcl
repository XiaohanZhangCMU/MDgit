...
set epVAL [ format %04d [expr int($epsilon*1000) ] ]
...
MD++ dirname = runs/ho-Ni-DN/${T}K_${epVAL}eps_crossslip_112
...
set dsig_xx [expr -1.0*( $targetStressMpa_xx -  $sigxx )] ; 
set dsig_yy [expr -1.0*( $targetStressMpa_yy -  $sigyy )]
set dsig_zz [expr -1.0*( $targetStressMpa_zz -  $sigzz )] ;
set dsig_xy [expr -1.0*( $targetStressMpa_xy -  $sigxy )]
set dsig_zx [expr -1.0*( $targetStressMpa_zx -  $sigzx )] ; 
set dsig_yz [expr -1.0*( $targetStressMpa_yz -  $sigyz )]
