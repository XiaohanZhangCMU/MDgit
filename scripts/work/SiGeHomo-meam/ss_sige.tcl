...
if { $status == 0 } {
   MD++ dirname = runs/ho-sige-DN-meam/0K_stressstrain
}  elseif { $status == 1 } {
   MD++ dirname = runs/ho-sige-DN-meam/${T}K_${epVAL}eps_112
} elseif { $status == 2 || $status == 22 } {
   MD++ dirname = runs/ho-sige-DN-meam/md_${T}K_${epVAL}eps_112-clean-$opt-$alt
} elseif { $status == 3 } {
   MD++ dirname = runs/ho-sige-DN-meam/mc_${T}K_${epVAL}eps_112
} elseif { $status == 44 } {
   MD++ dirname = runs/ho-sige-DN-meam/${T}K_${epVAL}eps_112/NEB
} elseif { $status == 4 } {
   MD++ dirname = runs/ho-sige-DN-meam/${T}K_${epVAL}eps_112/NEB
} elseif { $status == 5 || $status == 21 } { 
   MD++ dirname =  runs/ho-sige-DN-meam/${T}K_${epVAL}eps_112/UMB_$opt
} elseif { $status == 10 } { 
   MD++ dirname =  runs/ho-sige-DN-meam/view-${T}K_${epVAL}eps_112
}
exec cp -r /home/xzhang11/Planet/Libs/MD++.svn/scripts/work/SiGeHomo-meam/ss_sige.tcl .  
...
proc make_perfect_crystal_Ge { nx ny nz } {
    MD++ crystalstructure = diamond-cubic latticeconst = 5.650 #(A) for Ge
    MD++ latticesize = \[  1 -2 1  $nx  1 1 1  $ny  1 0 -1  $nz \]
    MD++ makecrystal #finalcnfile = perf.cn writecn #eval
}
...
proc make_alloy { nx ny nz } {

  set GeFrac 25
  puts "GeFrac = $GeFrac"

  make_perfect_crystal $nx $ny $nz
  MD++ nspecies=2 element0="Siz" element1="Ge5"
  MD++ atommass = \[ 28.0855 72.64 \]
  MD++ input = \[ 1 -10 10 -10 10 -10 10 \] fixatoms_by_position
# set all atoms to be Si
  MD++ input = 0 setfixedatomsspecies freeallatoms
  MD++ freeallatoms

  set NP [MD++_Get NP]
  set Nmax [expr $NP-1 ]
  set N_Ge [expr int($NP*$GeFrac/100.0)]
  set nums {}

  if {$N_Ge != $NP} {

     while {[llength $nums] < $N_Ge} {
         set n [myRand 0 $Nmax]
         if {$n ni $nums} {lappend nums $n}
     }

     for { set x 0 } { $x < $N_Ge } { incr x 1 } {
        set atomID [lindex $nums $x]
        MD++ input=\[ 1 $atomID \]
        MD++ fixatoms_by_ID
     }
  } elseif { $N_Ge == $NP } {
  ...
  MD++ input = 1 setfixedatomsspecies freeallatoms
  relax_fixbox
  MD++ finalcnfile = "Bulk_rand_SiGe_$GeFrac.cn" writecn
  MD++ finalcnfile = "Bulk_rand_SiGe_$GeFrac.cfg" writeatomeyecfg
  MD++ finalcnfile = "Bulk_rand_SiGe_$GeFrac.lammps" writeLAMMPS
  MD++ eval
}
...
#  make_alloy 6 5 10
  make_perfect_crystal_Ge 6 5 10
  setup_window
