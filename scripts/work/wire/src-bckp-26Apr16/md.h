...
//#define MAXCONSTRAINATOMS 40001 /* maximum number of atoms in constrained minimization */
#define MAXCONSTRAINATOMS 200001 /* maximum number of atoms in constrained minimization */
#endif
...
class POSCARFile : public AUXFile /* POSCAR (vasp) file */
{
public:
    POSCARFile(int t):AUXFile(t){};
    virtual char * describe();
private:
    virtual int writeblock(void *p);
    virtual int readblock(void *p);
...
class AVGCNFile : public AUXFile
{
public:
    AVGCNFile(int t):AUXFile(t){};
private:
    virtual int writeblock(void *p);
    virtual int readblock(void *p);
};
...
//    int usescalevelocity;        /* 1: if scale velocity for temperature control */
    ...
    double extforce[100];        /* external forces on different atom groups */
...
//    double BNHMass[MAXNHCLEN];         
    ...
    class Vector3 **_Rc, **_Fc; /* atom positions and forces along a chain of states */
    double *_nebinterp;
    int nebspec[10];
    int readrchainspec;
    double annealspec[10];
  
    /* Surface Tension Calculation (Seunghwa Ryu), only works for cubic cell */
    int    SURFTEN;
    int    surftensionspec[10];
    ...
    int _TORSIONSIM;
    double torquespec[3];           /* torsion specification */
    double _TORQUE, *_TORQUE_IND;
    int _BENDSIM;
    double bendspec[4];             /* bending specification */
    ...
    class AVGCNFile avgcn;/* file object that contains running averaged position */
    ...
    /* Interface to VASP */
    class POSCARFile initposcar,finalposcar;

    /* Interface to LAMMPS */
    class LAMMPSFile initlammps,finallammps;
    
    /* Interface to MDCASK */
    class MDCASKconFile initmdcaskcon,finalmdcaskcon;
    class MDCASKinputFile initmdcaskinput,finalmdcaskinput;
    char MDCASKpot[MAXFILENAMELEN];
    ...
    MDFrame(): _SAVEMEMORY(0),_NP(0),_NP0(0),_NIMAGES(0),_NPfree(0),_NPfixed(0),
               ...
               _SR1(0),_SR2(0),_SR3(0),_Rc(0),_Fc(0),_nebinterp(0), //_Fnebinterp(0),
               readrchainspec(-1),
    ...
    _qlmr(0),_qlmi(0),_qlm_r(0),_qlm_i(0),_qlm(0),qlm_id(1),
	       ...
	       wSKIN(0), DLxyz(0), DLNdiv(0),
               ...
               MCequilstep(0), UMB_K(0), UMB_equilstep(0), UMB_curstep(0), 
	       ...
	       continuecn(AUXFile::BLOCK),
               ...
               /* Interface to VASP */
               initposcar(AUXFile::BLOCK),finalposcar(AUXFile::BLOCK),
    ...
    virtual void SWITCHpotential_user_energyonly(double lambda);
...
#define refreshneighborlist NbrList_refresh
    ...
    virtual void MC_Recover();
     ...
     /* Monte Carlo subfunctions, umbrella sampling codes */
    virtual void MC_InitUMB();
    virtual void MC_AllocUMB();
    virtual void MC_RecoverUMB();
    virtual void MC_UpdateUMB();
    virtual void MC_UMBprocess();

    /* Monte Carlo subfunctions, forward flux sampling */ 
    ...
    int runMC();                    /* Metropolis Monte Carlo */
    int runTAMC();                  /* temperature accelerated Monte Carlo */
    int MCstep();
    int MCstep_moveatom();
    int MCstep_dV();
    ...
    /* Surface tension calculation */
    void AllocPtPn();
    void AddnvvtoPtPn(Vector3 &, Vector3 &, Vector3 &, double);
    void InitPtPn();
    void PrintPtPn();
    void InitSTsepa();
    ...
    void DFS2(int,int,int,double);
    void DFS1(int,int,double,double); 
    int ADVsample();
    int FFSprocess();
    void initFFShist();
    void logFFShist();
    ...
    void initUMB();
    void initKinetic();
    void UMBprocess();  /* umbrella sampling in MD, also see MC_UMBprocess() */
    int Kineticprocess();
    void setconfig4();
    void copyS4toS();
    void copyStoS4();
    ...
    void PME_init();
    void PME_clear();
    void PME_cal_bsp_fac();
    void PME_cal_bsp_Mk();
    double PME_cal_bsp_Mu(int n, double u);
    void PME_cal_bsp_Bm(int mi, int Ki);
    double PME_cal_bsp_Cm(int m1, int m2, int m3);
    ...
    int readcontinuecn();
    int readPOSCAR();
    int readOUTCAR(const char* filename);
    int readMDCASK();
    int readMDCASKJAIME();
    int readXYZ();
    int readLAMMPS();
