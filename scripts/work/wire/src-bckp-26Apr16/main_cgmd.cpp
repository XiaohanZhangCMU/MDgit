...
{                                                                    
    return sim.LJBONDFrame::Tcl_AppInit(interp);                                  
}                                                                    
...
{                                                                    
    ...
    fprintf(ostream,"\n");                                           
    fclose(ostream);                                                 
    sim.LJBONDFrame::parse_line(istream);                                         
    fclose(istream);                                                 
    return TCL_OK;                                                   
}                                                                    
...
{                                                                    
    int i, curn;                                                     
    curn = sim.LJBONDFrame::identify(argv[1]);                                    
    if(curn<0)                                                       
    {                                                                
    ...
    for(i=2;i<argc;i++)                                              
    {                                                                
        switch(sim.LJBONDFrame::vartype[curn])                                    
        {                                                            
        case(INT): sscanf(argv[i],"%d",(int *)sim.LJBONDFrame::varptr[curn]+i-2+sim.LJBONDFrame::shift); break; 
        case(LONG): sscanf(argv[i],"%ld",(long *)sim.LJBONDFrame::varptr[curn]+i-2+sim.LJBONDFrame::shift); break;
        case(DOUBLE): sscanf(argv[i],"%lf",(double *)sim.LJBONDFrame::varptr[curn]+i-2+sim.LJBONDFrame::shift); break;
        case(STRING): strcpy((char *)sim.LJBONDFrame::varptr[curn]+i-2+sim.LJBONDFrame::shift,argv[i]); break;   
        default: FATAL("unknown vartype ("<<sim.LJBONDFrame::vartype[curn]<<")"); 
        }                                                            
    }                                                                
    return TCL_OK;                                                   
}                                                                    
...
{                                                                    
    ...
    {                                                                
        ...
        switch(sim.LJBONDFrame::vartype[curn])                                    
        {                                                            
        case(INT): sprintf(buf,"%d",*((int *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
        case(LONG): sprintf(buf,"%ld",*((long *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break;
        case(DOUBLE): sprintf(buf,"%.16g",*((double *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break;
        case(STRING): sprintf(buf,"%s",(char *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift); break;
        default: FATAL("unknown vartype ("<<sim.LJBONDFrame::vartype[curn]<<")"); 
        }                                                            
    }                                                                
...
md_cmd(PyObject *self, PyObject *args)
{
    ...
    fprintf(ostream,"\n");                                           
    fclose(ostream);                                                 
    ret_val = sim.LJBONDFrame::parse_line(istream);                                         
    fclose(istream);                                                 
...
{                       
    ...
    curn = sim.LJBONDFrame::identify(s);                                    
    if(curn<0)                                                       
    {                                                                
    ...
    switch(sim.LJBONDFrame::vartype[curn])                                    
    {                                                            
    case(INT): return Py_BuildValue("i", *((int *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
    case(LONG): return Py_BuildValue("l", *((long *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
    case(DOUBLE): return Py_BuildValue("d", *((double *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
    case(STRING): return Py_BuildValue("s", ((char *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
    default: FATAL("unknown vartype ("<<sim.LJBONDFrame::vartype[curn]<<")"); return NULL;
    }                                                            
    return NULL;                                                   
}                                                                    
...
{                       
    ...
    for (offset=istart; offset<iend; offset+=iskip)
    {
        switch(sim.LJBONDFrame::vartype[curn])                                    
        {                                                            
        case(INT): num = Py_BuildValue("i", *((int *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
        case(LONG): num = Py_BuildValue("l", *((long *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
        case(DOUBLE): num = Py_BuildValue("d", *((double *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
        case(STRING): num = Py_BuildValue("s", ((char *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift)); break; 
        default: FATAL("unknown vartype ("<<sim.LJBONDFrame::vartype[curn]<<")"); return NULL;
        }                                                            
        if (!num) {
            Py_DECREF(lst);
...
{                       
    ...
    for (i = 0; i<numLines; i++)
    {
        ...
        switch(sim.LJBONDFrame::vartype[curn])                                    
        {                                                            
        case(INT): *((int *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift) = (int) PyInt_AsLong(num); break; 
        case(LONG): *((long *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift) = PyInt_AsLong(num); break; 
        case(DOUBLE): *((double *)sim.LJBONDFrame::varptr[curn]+offset+sim.LJBONDFrame::shift) = PyFloat_AsDouble(num); break; 
        case(STRING): strcpy((char *)sim.LJBONDFrame::varptr[curn]+i-2+sim.LJBONDFrame::shift,PyString_AsString(num)); break;   
        default: FATAL("unknown vartype ("<<sim.LJBONDFrame::vartype[curn]<<")"); return NULL;
        }                                                            
        offset += iskip; 
   }
...
{
    sim.LJBONDFrame::ParallelInit(&argc,&argv);

    if(sim.LJBONDFrame::myDomain==0)
...
int main_slave(int argc, char *argv[])
{
    sim.LJBONDFrame::initvars();
    strcpy(sim.LJBONDFrame::myname,argv[0]);
    
    sim.LJBONDFrame::WaitForCommand();
    return 0;
}
#else
...
int main_master(int argc, char *argv[])
{
  ...
  Py_InitModule("mdpp", MDMethods);
    ...
    if (argc>2) 
    {
       sim.LJBONDFrame::set_dirname(argv[2]);
    }
    if (argc>3) sim.LJBONDFrame::set_randseed(argv[3]);
    sim.LJBONDFrame::parse(SCParser::getfilehandler(argc,argv));
#endif//_USEPY
#else
