...
void FEMFrame::fem_energy_force()
{
  ...
  if (myDomain == 1 || myDomain == 0 && 0) {
    int size = _NP*allocmultiple;
    //INFO_Printf("broadcast[%d]; F = [ %f %f \n      %f %f ]\n",myDomain, Fdef[0][0],Fdef[0][1],Fdef[1][0],Fdef[1][1]);
    INFO_Printf("broadcast[%d]; trace = %f, gauss[0]= %f\n",myDomain, trace, gauss_weight[0]);
    // for (int i = 0;i<_NP;i++) {
    ...
    // INFO_Printf("fem_potential[%d], _EPOT = %lf\n",myDomain, _EPOT);

    // for(j=0;j<2;j++)
    //   INFO_Printf("fem_force[%d], F[%d] = [%f,%f,%f]\n",myDomain, j, _F[j][0],_F[j][1],_F[j][2]);
