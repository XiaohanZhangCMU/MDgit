...
# meam-lammps_mc2_mpich scripts/disl_nuc_hetero.tcl 0 0 001 1     -generates strained perfect thin film --> state A.
# meam-lammps_mc2_mpich scripts/disl_nuc_hetero.tcl 1 0.030 001 1 -generates shuffle glide dislcoaiton -->  state B.
# mpirun -np $ncpu meam-lammps_mc2_mpich scripts/disl_nuc_hetero.tcl 4 0.030 001 1 ----string_relax_parallel.
# meam-lammps_mc2_mpich scripts/disl_nuc_hetero.tcl 20 0.030 001 1 ---- visualize relaxed chain.
...
MD++ dirname = runs/he-si-tf-pit-neb/he-si-tf-pit-special-3-meam
#max number neigbors
exec cp ../../../scripts/work/he-si-tf-pit-neb/he-si-tf-pit-special-3-meam/disl_nuc_hetero.tcl .
...
MD++ nspecies = 1  element0 = "Siz" element1 = "Ge" 
MD++ {
rcut = 4.5  readMEAM
NNM = 300
} }
...
proc make_glide_dislocation_loop_1 { flag epsilon } { 
    ...
    if { $flag == 001 } {
       ...
       if { $epsilon <= 0.030 } {
	       set w [expr $Ly * 0.495]
       } elseif { $epsilon <= 0.0490 } {
	       set w [expr $Ly * 0.4866]
       } elseif { $epsilon <= 0.0500 } {
	       set w [expr $Ly * 0.4864]
       } elseif { $epsilon <= 0.0510 } {
	       set w [expr $Ly * 0.4862]
       } elseif { $epsilon <= 0.0516 } {
	       set w [expr $Ly * 0.4865]
       } elseif { $epsilon <= 0.0518 } {
	       set w [expr $Ly * 0.4863]
...
#       set RDx  0.7072
       ...
       set P0y [expr  0.0 * $Ly ]
       set P0x [expr  0.5*(0.2708+0.2916) * $Lx]
       set P0z [expr  0.5*(0.2605+0.2530) * $Lz ]
...
#       set RDx  0.7072
       ...
       set P0y [expr  0.0* $Ly  ]
       set P0x [expr  0.5*(0.2500+0.2753) * $Lx]
       set P0z [expr  0.5*(0.2708+0.2679) * $Lz ]
...
#       set RDx  0.7072
       ...
       set P0y [expr  0.0*$Ly  ]
       set P0x [expr  0.25*( 0.2708 + 0.2916 + 0.2500 + 0.2708 ) * $Lx]
       set P0z [expr  0.25*( 0.2605 + 0.2530 + 0.2753 + 0.2679 ) * $Lz ]
...
#  readpot
  ...
  exec ln -f -s ../he-si-tf-pit-special-3-sworig/0K_0.0_relaxed_surf${flag}.cn  sworig.cn
  MD++ incnfile = "sworig.cn" readcn 
  #setup_window
  #openwindow
  #MD++ plot
  relax_fixbox
...
# only do this once
  ...
  MD++  conj_ftol = 2e-8 conj_itmax = 1000 conj_fevalmax = 1000 conj_fixbox = 1
  MD++  relax
#  MD++ finalcnfile = "../w-disl-nuc-hetero-0/0K_0.0_relaxed_surf${flag}.cn" writecn
  MD++ sleep
  ...
  relax_fixbox
  MD++ finalcnfile = "../w-disl-nuc-hetero-0/0K_${epsilon}_relaxed_surf${flag}.cn" writecn
  }  
...
#  MD++ finalcnfile = "w-loop-relaxed-surf${flag}.cfg" writeatomeyecfg
  ...
  MD++ conj_ftol = 1e-4 conj_itmax = 10 conj_fevalmax = 300

  # this is to confirm the structure is beyond critical
  # MD++ conj_ftol = 1e-4 conj_itmax = 3800 conj_fevalmax = 5000
...
#  setup_window
  ...
  MD++ chainlength = $chainlength  
  #MD++ timestep = 0.01 printfreq = 10
  MD++ timestep = 0.001 printfreq = 10
  ...
  ## [ relax_surround redistr_freq moveleftend moverightend yesclimbimage islengthconstant ]
   ...
   MD++ nebspec =  \[ 0 1 -1 3 0 0 1  0 0 0  1 10 \] totalsteps = 100000 equilsteps = 10000
...
#  readpot
  ...
  MD++ incnfile = "../../../he-si-tf-pit-special-3/runs/w-disl-nuc-hetero-${flag}-${epsilon}.24/meam_relaxed_chain_no_0.cn" readcn
#  MD++ incnfile = "../w-disl-nuc-hetero-0/0K_${epsilon}_relaxed_surf${flag}.cn" 
  MD++ nspecies = 1  element0 = "Siz"
  set_all_atoms_species 0 
  MD++ saveH
  MD++ eval
#  MD++ incnfile = "../w-disl-nuc-hetero-0/w-loop-compression-surf${flag}-eps${epsilon}.cn" readcn   
  MD++ incnfile = " ../w-disl-nuc-hetero-relax-path-0.0450/chain_no_23.cn" readcn
  MD++ nspecies = 1  element0 = "Siz"
  set_all_atoms_species 0 
  MD++  restoreH SHtoR  setconfig2
  MD++ eval
#  MD++ incnfile = "../w-disl-nuc-hetero-0/0K_${epsilon}_relaxed_surf${flag}.cn" readcn
...
#  setup_window
  ...
  ## [ relax_surround redistr_freq moveleftend moverightend yesclimbimage islengthconstant ]
    ...
    #MD++ nebspec = \[ 0 1 0 1 0 0 1 \] totalsteps = 100000 equilsteps = 10000
    MD++ nebspec = \[ 0 1 0 1 0 0 1 \] totalsteps = 100 
  }   

  MD++ Broadcast_nebsetting
  MD++ Broadcast_MEAM_Param
  MD++ fixallatoms constrain_fixedatoms freeallatoms
  MD++ allocchain_parallel 
  MD++ initRchain_parallel
  ...
  MD++ eval
  #MD++ stringrelax_parallel
  for { set iter 1 } { $iter <= 400 } { incr iter } {
    MD++ stringrelax_parallel_2
    file mkdir "saved-path-$iter"
    MD++ finalcnfile = "saved-path-$iter/stringrelax.chain.cn" writeRchain_parallel
    file rename -force "stringeng.out" "saved-path-$iter/stringeng.out" 
     
#    MD++ finalcnfile = stringrelax.chain.cn writeRchain_parallel
#    set chain_files [split [glob *.chain.cn.*]]
#    file mkdir "saved-path-$iter"
#    foreach f $chain_files {
#       file rename $f "saved-path-$iter" 
#    }
  }
  if { 0 } {
  exec cp stringeng.out stringeng_step1.out
...
#  readpot
  ...
  MD++ incnfile = "../w-disl-nuc-hetero-0/0K_${epsilon}_relaxed_surf${flag}.cn" readcn  saveH
  MD++ zipfiles = 0
  for { set sid 190 } { ${sid} <= 190 } {incr sid 1 } {
      file mkdir "saved-path-${sid}"
      set infile00 "../w-disl-nuc-hetero-${flag}-${epsilon}.24/saved-path-${sid}/stringrelax.chain.cn.cpu00" 
      set infile "../w-disl-nuc-hetero-${flag}-${epsilon}.24/saved-path-${sid}/stringrelax.chain.cn" 

      set check [ glob -nocomplain $infile00 ]
      set exist [ llength $check ]
      puts "check = $check"
      puts "exist = $exist"
      if { $exist < 1 } {
...
#             openwindow
         ...
         # write cn or cfg file
         MD++ finalcnfile = "saved-path-${sid}/chain_no_${chain_no}.cn"  writecn
         MD++ finalcnfile = "saved-path-${sid}/chain_no_${chain_no}.cfg" writeatomeyecfg
         MD++ sleepseconds = 1  sleep
...
#  readpot
  ...
  MD++ incnfile = "../../../he-si-tf-pit-special-3/runs/w-disl-nuc-hetero-0/0K_${epsilon}_relaxed_surf${flag}.cn" readcn  saveH
  MD++ zipfiles = 0

  set infile00 "../../../he-si-tf-pit-special-3/runs/w-disl-nuc-hetero-${flag}-${epsilon}.24/stringrelax.chain.cn.cpu00" 
  set infile "../../../he-si-tf-pit-special-3/runs/w-disl-nuc-hetero-${flag}-${epsilon}.24/stringrelax.chain.cn" 
  ...
  # Relax state A of SW with meam
  MD++ input = 0 incnfile = $infile
  set chain_no 0
  MD++ input = $chain_no  readRchain_parallel_toCN  RHtoS
  relax_fixbox
  MD++ finalcnfile = "../../../he-si-tf-pit-special-3/runs/w-disl-nuc-hetero-${flag}-${epsilon}.24/meam_relaxed_chain_no_0.cn" writecn

  # Generate meam initial path from sw, do switchconfig for each node to amend energy difference of state A of SW and MEAM. 
  MD++ incnfile = $infile input = 0  readRchain_parallel_toCN  RHtoS setconfig1
  MD++ incnfile = "../../../he-si-tf-pit-special-3/runs/w-disl-nuc-hetero-${flag}-${epsilon}.24/meam_relaxed_chain_no_0.cn" readcn setconfig2
  MD++ incnfile = $infile
  set fp [ open "EPOT.dat" a+ ]
  for { set iter 0 } { $iter <= $total_no } { incr iter 1 } {
     set chain_no $iter
     MD++ input = $chain_no  readRchain_parallel_toCN  RHtoS
#   translate sw relaxed path to start from meam relaxed state. 
#   dislocation nucleation from si surface with meam potential.
     MD++ switchconfig
     MD++ finalcnfile = "chain_no_${chain_no}.cn"  writecn
     MD++ finalcnfile = "chain_no_${chain_no}.cfg" writeatomeyecfg
     MD++ finalcnfile = "stringrelax.chain.cn" input = $iter writeRchain_parallel_fromCN 
...
#  readpot
  ...
  #set chainfile "../w-disl-nuc-hetero-${flag}-${epsilon}.$ncpu.saved/saved-path-399/stringrelax.chain.cn" 
  set chainfile "../w-disl-nuc-hetero-${flag}-${epsilon}.$ncpu/saved-path-399/stringrelax.chain.cn" 
...
#  make_perfect_crystal 30 15 42
  ...
  MD++ conj_fixbox = 1 conj_ftol = 2e-6 conj_itmax = 1000 conj_fevalmax = 2000
  MD++ relax finalcnfile = "0K_perfect.cn" writecn
  MD++ finalcnfile = "0K_perfect.cfg" writeatomeyecfg
  ...
  if {$make_free_surface} {
    ...
    if { $flag == 001 } {
       ...
       MD++ finalcnfile = "0K_surf${flag}.cfg" writeatomeyecfg
...
MD++ incnfile = "0K_surf${flag}.cn" readcn
#setup_window
#openwindow
...
MD++ crystalstructure = diamond-cubic latticeconst =  5.4309529817532409 #(A) for Si
MD++ latticesize = \[  1 1 0  12  -1 1 0  12  0 0 1  21 \]
...
if { $surface_reconstruct } {
 ...
 for { set i 0 } { $i < $ny } { incr i 1 } {
    set ymin [ expr -0.500+1.0/$ny*$i ]
    set ymax [ expr -0.500+1.0/$ny*($i+0.5) ]
 ...
 for { set i 0 } { $i < $ny } { incr i 1 } {
    set ymin [ expr -0.500+1.0/$ny*($i+0.5) ]
    set ymax [ expr -0.500+1.0/$ny*($i+1) ]
    MD++ input = \[ 1 -10 10 $ymin $ymax 0.3025   10 \]  fixatoms_by_position
    MD++ input = \[ 1 -10 10 $ymin $ymax -10 -0.310 \]  fixatoms_by_position
    MD++ input = \[ 1 -10 10 $ymin $ymax $zholemin_tol $zholemax_tol \]  fixatoms_by_position
 }
 ...
 MD++ finalcnfile = "0K_trench${flag}_reconstr.cfg"  writeatomeyecfg

# MD++ eval plot  sleep 
  relax_fixbox
  MD++ finalcnfile = "0K_trench${flag}.cn"  writecn
  MD++ finalcnfile = "0K_trench${flag}.cfg"  writeatomeyecfg
