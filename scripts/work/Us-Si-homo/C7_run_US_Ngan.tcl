...
#MD++ setnolog
MD++ setoverwrite
MD++ dirname = runs/ho-cu-DN/${T}K_${P}eps_112/UMBold_${n}
MD++ srand48bytime
MD++ srandbytime
read_potential
}
...
proc read_lam { } {
   set fp [ open "../NEB_old/lam_array.dat" r ]
   set data [ read $fp ]
   close $fp
   return $data 
}
...
proc get_N { } {
   ...
   for { set k 0 } { $k <= 200 } { incr k } {
	...
	set check [ glob -nocomplain ../NEB_old/$filename ]
...
#           puts "$j $lam_cur_n $current_n"
        ...
        set fp [ open "../UMBold_arg${i}.txt" w ]
#        puts ".."
#        puts $fp $i
        puts $fp $lam_int
#	set delta_n 15
...
proc setup_window { n } {
    ...
    while { 1 } {
    set check [ glob -nocomplain ../UMBold_arg${n}.txt ]
    set NUM [ llength $check ]
    if { $NUM > 0 } {
	break;
    }
    ...
    set fp [ open "../UMBold_arg${n}.txt" r ]
...
MD++ incnfile = "../NEB_old/run_${n}/FFS_${n}_0001.cn" readcn
...
# touch only if you want to use different normalization
if { $n <= 1 } {
  set direc_no 0
	if { 0 } {
	    MD++ UMB_K = -1
