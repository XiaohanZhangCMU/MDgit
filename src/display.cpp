/*
  ...
  Last Modified : Sat Dec 17 21:07:55 2005
...
#endif //end of ifndef NO_X11
}

void SYWindow::Draw3DArrow(YPolygon polygon)
{
#ifndef NO_X11
    XPoint points[4];
    double x1,y1,x2,y2,x3,y3; unsigned attr;
    double xs1, ys1, zs1, xs2, ys2, zs2, xs3, ys3, zs3, Z;
    unsigned long col=cblack;
    ...
    PBCSHIFT(xs3,pbcshift[0],xs1);
    PBCSHIFT(ys3,-pbcshift[1],ys1);
    PBCSHIFT(zs3,pbcshift[2],zs1);
    
    Z=A31*xs3+A32*ys3+A33*zs3;
    x3=CX(A11*xs3+A12*ys3+A13*zs3,Z);
    y3=CY(A21*xs3+A22*ys3+A23*zs3,Z);

    points[0].x = (short int)round(x1); points[0].y = (short int)round(y1); 
    points[1].x = (short int)round(x2); points[1].y = (short int)round(y2);  
    points[2].x = (short int)round(x3); points[2].y = (short int)round(y3);  
    points[3].x = (short int)round(x1); points[3].y = (short int)round(y1);  

    //INFO("test");
    //INFO("polygon.c = "<<(polygon.c==col));
    if(col!=polygon.c)
    {
        XSetForeground(theDisplay, gc, col=polygon.c);
        //r=CR(polygon.r,0);
        //if(r<1) r=1;
        //if(r>maxlinewidth) r=maxlinewidth;
            
        XFillPolygon(theDisplay, pixmap, gc, points, 3, Convex, CoordModeOrigin);
        //XSetLineAttributes(theDisplay, gc, (int)r,
        //                   LineSolid, CapRound, JoinRound);
        XSetForeground(theDisplay, gc, cblack);
        XDrawLines(theDisplay, pixmap, gc, points, 3, CoordModeOrigin);
    }
#endif //end of ifndef NO_X11
}
...
//    DUMP("YWindow::paint()");
    if (sort)
    {
        ...
        for(i=0;i<nPoly;i++)
        {
            xs=(Polygons[i].x0+Polygons[i].x1+Polygons[i].x2)/3;
            ys=(Polygons[i].y0+Polygons[i].y1+Polygons[i].y2)/3;
            zs=(Polygons[i].z0+Polygons[i].z1+Polygons[i].z2)/3;

            PBCSHIFT(xs,pbcshift[0],0);
            PBCSHIFT(ys,-pbcshift[1],0);
            PBCSHIFT(zs,pbcshift[2],0);
            
            Z_ind[i+nL+nP].Z=A31*xs+A32*ys+A33*zs;
            Z_ind[i+nL+nP].index=i+nL+nP;
        }
        //printf("qsort: nL=%d nP=%d nPoly=%d\n",nL,nP,nPoly);
        qsort(Z_ind, nL+nP+nPoly, sizeof(struct ZInd), &cmp);
        
        for(i=0;i<nL+nP+nPoly;i++)
        {
            int n=Z_ind[i].index;
            if(n<nL) Draw3DLine(Lines[n]);
            else if(n<(nL+nP)) Draw3DPixel(Points[n-nL]);
            else if(n<(nL+nP+nPoly)) Draw3DArrow(Polygons[n-nL-nP]); 
        }
    }
    else
    {
...
void SYWindow::Evolve()
{
    ...
    while(XPending(theDisplay))
    {
        ...
        case KeyPress:
            ...
            ks = XkbKeycodeToKeysym(theDisplay,ev.xkey.keycode,0,0);
            switch (ks)
            {
            case XK_Escape:
                FreeResource(); return;
...
void SYWindow::DrawArrow(double x0, double y0, double z0, 
                         double x1, double y1, double z1, unsigned long co,
                         double r, double ratio, unsigned long attr)
{
    double dx, dy, dz, xi, yi, zi, d, r0;
    double alpha, c, s, pos[6][3];
    double nx, ny, nz, n;
    ...
    d = dx*dx + dy*dy + dz*dz;
    d = sqrt(d); r0 = ratio*d*.4;

    /* alpha : angle between [x1-x0,y1-y0,z1-z0] and [0,0,d] */
    c = (z1-z0)/d; alpha = acos(c); s = sin(alpha);
    /* nx, ny, nz : 
       three components of the normal vector to the plane 
       defined by [x1-x0,y1-y0,z1-z0] and [0,0,d] */
    ...
    {
        nx = dy*d; ny = -dx*d; nz = 0.0;
        n = nx*nx + ny*ny + nz*nz; n = sqrt(n);
        nx = nx/n; ny = ny/n;
        /* Rotation matrix around [nx,ny,nz] by alpha */
        R11 = c + nx*nx*(1-c); R12 = nx*ny*(1-c)-nz*s; R13 = nx*nz*(1-c)+ny*s;
        R21 = ny*nx*(1-c)+nz*s; R22 = c + ny*ny*(1-c); R23 = ny*nz*(1-c)-nx*s;
        R31 = nz*nx*(1-c)-ny*s; R32 = nz*ny*(1-c)+nx*s; R33 = c + nz*nz*(1-c);
    }

    if(nPoly<MaxPolygons-1)
    { 
        for(i=0;i<6;i++)
        {
            xi=r0*cos(M_PI/3.0*i); yi=r0*sin(M_PI/3.0*i); zi = -1.0*(ratio)*d;
            pos[i][0] = R11*xi+R21*yi+R31*zi + x1; 
            pos[i][1] = R12*xi+R22*yi+R32*zi + y1;
            pos[i][2] = R13*xi+R23*yi+R33*zi + z1;
        }
        for(i=0;i<6;i++)
        {
        Polygons[nPoly+i].x0=x1; Polygons[nPoly+i].y0=y1; Polygons[nPoly+i].z0=z1; 
        Polygons[nPoly+i].x3=x1; Polygons[nPoly+i].y3=y1; Polygons[nPoly+i].z3=z1; 
        Polygons[nPoly+i].x1=pos[i][0]; Polygons[nPoly+i].y1=pos[i][1]; Polygons[nPoly+i].z1=pos[i][2];
            if(i<5)
	    {
                Polygons[nPoly+i].x2=pos[i+1][0]; Polygons[nPoly+i].y2=pos[i+1][1]; Polygons[nPoly+i].z2=pos[i+1][2];
	    }
            else
	    {
                Polygons[nPoly+i].x2=pos[0][0]; Polygons[nPoly+i].y2=pos[0][1]; Polygons[nPoly+i].z2=pos[0][2];
	    }

            Polygons[nPoly+i].c=co;
            Polygons[nPoly+i].attr=attr;
            //printf("nPoly+i=%d\n",nPoly+i);
        }
        nPoly+=6;
        //printf("nPoly=%d\n",nPoly);
    ...
    {
        WARNING("SYWindow: Too many polygons (more than "
                 << (int)MaxPolygons
                 << "), ignoring");
    }    
}
...
#ifdef _GENERAL_H
    INFO("PixDepth="<<PixDepth);
#else
    printf("PixDepth=%d",PixDepth);
#endif
    if(PixDepth==16)
    {
        for(i=0;i<3;i++)
...
#endif
            case XK_a:msrsp=ASPECTRATIO;dirty=true;break;
            case XK_d:msrsp=PROJECTION;dirty=true;break;
            case XK_s:msrsp=SCALING;dirty=true;break;
            case XK_t:msrsp=TRANSLATION;dirty=true;break;
            case XK_r:msrsp=ROTATION;dirty=true;break;
