...
{
    int fiber_type, nfiber, chainlen, fix_bottom;
    int ipt, i, j;
    ...
    if(_VR!=NULL)
    {
	  ...
	  /* xiaohan: this removes the interaction between substrate with fe nodes which have species = 2*/
	    ...
	    continue;
	      ...
	      //INFO_Printf("katm = %lf\n",tmp*_VR[i].norm2()*_ATOMMASS[species[i]]/_ATOMMASS[0]);
	      // INFO_Printf("pstress = %lf, %lf, %lf\n", _PSTRESS[0], _PSTRESS[1], _PSTRESS[2]);
          }
      }
    }
